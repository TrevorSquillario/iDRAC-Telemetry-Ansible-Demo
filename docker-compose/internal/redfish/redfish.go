...
func (r *RedfishClient) ListenForMetricReports(Ctx context.Context, event chan<- *RedfishEvent) {
	...
	if err == nil {
		...
		if err == nil {
			if eventService.Object["ServerSentEventUri"] != nil {
				ret.Err = r.GetMetricReportSSE(Ctx, event, eventService)
...
func (r *RedfishClient) GetMetricReportsSSE(Ctx context.Context, event chan<- *RedfishEvent, sseURI string) error {
	...
	sseConfig.RequestCreator = func() *http.Request {
		sseMetricReportUri := sseURI+"?$filter=EventFormatType%20eq%20MetricReport"
		log.Printf("Metric Report SSE URI %v", sseMetricReportUri)
		req, err := http.NewRequest("GET", sseMetricReportUri, nil)
		if err != nil {
			return nil
		}
		r.addAuthToRequest(req)
	...
	for {
		...
		default:
			...
			if err != nil {
				log.Println("Error reading metric report!", err)
				break
			}
...
func (r *RedfishClient) GetEventsSSE(Ctx context.Context, event chan<- *RedfishEvent, sseURI string) error {
	...
	sseConfig.Client = r.HttpClient
	sseConfig.RequestCreator = func() *http.Request {
		sseAlertUri := sseURI+"?$filter=EventFormatType%20eq%20Event"
		log.Printf("Alert Event SSE URI %v", sseAlertUri)
		req, err := http.NewRequest("GET", sseAlertUri, nil)
		if err != nil {
			return nil
		}
		r.addAuthToRequest(req)
	...
	for {
		...
		default:
			sseEvent, err := sseSource.Next()
			if err != nil {
				log.Println("Error reading alert events!", err)
				break
			}
			redfishEvent := new(RedfishEvent)
			redfishEvent.ID = sseEvent.ID
			...
			ret.Client = r
			redfishEvent.Payload = ret
			log.Printf("Alert Event SSE payload %v", ret)
			event <- redfishEvent
		}
	}
}
...
func (r *RedfishClient) GetEventSSE(Ctx context.Context, event chan<- *RedfishEvent, eventService *RedfishPayload) error {
	sseUri := "https://" + r.Hostname + eventService.Object["ServerSentEventUri"].(string)
	return r.GetEventsSSE(Ctx, event, sseUri)
}

func (r *RedfishClient) GetMetricReportSSE(Ctx context.Context, event chan<- *RedfishEvent, eventService *RedfishPayload) error {
	sseUri := "https://" + r.Hostname + eventService.Object["ServerSentEventUri"].(string)
