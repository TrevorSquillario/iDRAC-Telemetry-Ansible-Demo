...
func parseReport(metricReport *redfish.RedfishPayload, systemid string, hosttags string, hostname string, dataBusService *databus.DataBusService) {
	...
	group.Label = "Redfish Metric Report" //metricReport.Object["Name"].(string)
	group.HostTags = hosttags
	group.HostName = hostname
...
func (r *RedfishDevice) RestartMetricReportEventListener() {
...
// StartMetricReportEventListener Directly responsible for receiving SSE events from iDRAC. Will parse received reports or issue a
// message in the log indicating it received an unknown SSE event.
func (r *RedfishDevice) StartMetricReportEventListener(dataBusService *databus.DataBusService) {
	if r.Events == nil {
		r.Events = make(chan *redfish.RedfishEvent, 10)
	}
	//timer := time.AfterFunc(time.Minute*5, r.RestartMetricReportEventListener)
	log.Printf("%s: Starting metric report event listener...\n", r.SystemID)
	go r.Redfish.ListenForMetricReports(r.Ctx, r.Events)
	for {
		event := <-r.Events
		if event == nil {
		...
		//log.Printf("StartMetricReportEventListener %#v\n", event.Payload)
		r.LastEvent = time.Now()
		if event != nil && event.Payload != nil && event.Payload.Object["@odata.id"] != nil {
			if !strings.Contains(event.Payload.Object["@odata.type"].(string), "MetricReport")  {
				continue
			}
			log.Printf("%s: Got new report for %s\n", r.SystemID, event.Payload.Object["@odata.id"].(string))
			parseReport(event.Payload, r.SystemID, r.HostTags, r.Redfish.Hostname, dataBusService)
		} else {
			//log.Printf("%s: Got unknown SSE event %v\n", r.SystemID, event.Payload)
			log.Printf("%s: Got unknown SSE event \n", r.SystemID)
		}
...
func (r *RedfishDevice) StartLceEventListener(dataBusService *databus.DataBusService) {
	...
	log.Printf("%s: Starting RLCE event listener...\n", r.SystemID)
	go r.Redfish.ListenForLceEvents(r.Ctx, r.Events)
	for {
		lceevent := <-r.Events
		timer.Reset(time.Minute * 5)
...
func getTelemetry(r *RedfishDevice, telemetryService *redfish.RedfishPayload, dataBusService *databus.DataBusService) {
	...
	for i := 0; i < size; i++ {
		...
		parseReport(metricReport, r.SystemID, r.HostTags, r.Redfish.Hostname, dataBusService)
	}
	r.State = databus.RUNNING
	r.StartMetricReportEventListener(dataBusService)
}
...
func redfishMonitorStart(r *RedfishDevice, dataBusService *databus.DataBusService) {
	...
	if err != nil {
		...
		log.Printf("%s: Telemetry Service consumption loading...\n", r.Redfish.Hostname)
...
func main() {
	...
	for {
		...
		if err != nil {
			...
			log.Printf("Connected to message bus: %s:%s", configStrings["mbhost"], configStrings["mbport"])
			authClient.Bus = mb
